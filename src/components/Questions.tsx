...
const sample = Array.from({ length: 6 }).map((_, i) => ({
  id: i + 1,
  text: `Question example #${i + 1}: Solve for x in the equation ...`,
        ...
        {sample.map((q) => (
          <View key={q.id} style={styles.card}>
            <View style={styles.cardHead}>
              ...
              <Text style={[styles.badge, q.status === 'Active' ? styles.badgeActive : q.status === 'Draft' ? styles.badgeDraft : styles.badgePending]}>
                {q.status}
              </Text>
            </View>
            <Text style={styles.cardMeta}>Updated {q.updated}</Text>
            <View style={styles.cardActions}>
              ...
              <TouchableOpacity style={styles.smallBtn}>
                <Text style={styles.smallText}>Archive</Text>
              </TouchableOpacity>
            </View>
          </View>
        ))}
...
const styles = StyleSheet.create({
  ...
  cardMeta: { color: colors.textMuted, marginBottom: spacing.sm },
  cardActions: { flexDirection: 'row', justifyContent: 'flex-end', gap: spacing.sm },
  linkBtn: { padding: spacing.xs },
  smallBtn: { padding: spacing.xs, backgroundColor: 'transparent' },
